...
with col2:
    ...
    lease_commence_date	=st.number_input('Enter the **Lease Commence Year**', min_value = 1966, max_value= 2022, value = 2017 )
...
st.markdown('Click the button to predict **Flat Resale Price**')
prediction = st.button('**Predict**')



if prediction and test_data:
    predicted_price = regression_model(test_data)
    st.markdown(f"### :red[Flat Resale Price is] :green[${round(predicted_price, 3)}]  :orange[ SGD]")

    inr_price = round(predicted_price * 61.99, 3)
    st.markdown(f"### :red[Flat Resale Price is] :green[₹ {inr_price}]  :orange[ INR]")
